...
const userExists = async (email) => {
    const query = 'SELECT * FROM hd_users WHERE emp_id = $1';
    const values = [email];
    try {
...
const addNewUser = async (userObj) => {
    const { name, email, password, user_type, dob, doj,phone } = userObj;
    const query = 'INSERT INTO hd_users (name, email,emp_id, password, user_type, dob, doj,phone) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING*';
    const values = [name, email,email, password, user_type, dob, doj,phone];
    try {
        const results = await pool.query(query, values);
